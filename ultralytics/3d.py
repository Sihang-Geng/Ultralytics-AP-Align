...
def compute_visuals_v2(image_path, output_root):
    ...
    # --- 文件名中文，图内英文 ---
    save_3d_surface(
        grad_norm, "Raw Gradient Landscape", os.path.join(save_dir, "1_3d_原始梯度.jpg"), z_label="Gradient Magnitude"
    )
    save_3d_surface(
        grad_lcgr_norm,
        "LCGR Enhanced Landscape",
        os.path.join(save_dir, "2_3d_LCGR增强.jpg"),
        z_label="Enhanced Magnitude",
    )

    h, w = gray.shape
    center_roi_x = sobelx[h // 2 - 50 : h // 2 + 50, w // 2 - 50 : w // 2 + 50]
    center_roi_y = sobely[h // 2 - 50 : h // 2 + 50, w // 2 - 50 : w // 2 + 50]
    save_polar_histogram(
        center_roi_x, center_roi_y, "Gradient Orientation Distribution", os.path.join(save_dir, "3_极坐标_方向分布.jpg")
    )
